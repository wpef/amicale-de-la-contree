 ...
 */
function mustGoHigherTrump(
  availableTrumps: Card[],
  trick: Trick,
  trumpSuit: Suit,
): Card[] {
  ...
  const higherTrumps = availableTrumps.filter(
    (c) => cardStrength(c, trumpSuit) > highestTrumpInTrick,
  );
...
function getHighestTrumpStrength(trick: Trick, trumpSuit: Suit): number | null {
...
function isPartnerWinning(trick: Trick, trumpSuit: Suit, playerSeat: Seat): boolean {
  if (trick.cards.length === 0) return false;

  // Build a partial trick to determine current winner
  const partialTrick: Trick = {
    ...trick,
    cards: trick.cards,
  };
...
function determineTrickWinnerPartial(trick: Trick, trumpSuit: Suit): Seat | null {
