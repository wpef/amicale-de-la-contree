import { parseCardId, cardEquals } from '../domain/card.js';
import { TeamId, teamForSeat } from '../domain/team.js';
import { Seat, nextSeat, SEAT_ORDER } from '../utils/seat.js';
...
import { isCardPlayable } from '../rules/play-validation.js';
...
function handleSetTargetScore(state, action) {
    ...
    if (action.score < 500 || action.score > 5000)
...
function handlePlayCard(state, action) {
    ...
    if (isTrickComplete(updatedTrick)) {
        ...
        if (newTricks.length === 8) {
            ...
            const beloteTeam = state.beloteHolder && state.beloteDeclared
