...
import { teamForSeat } from '../domain/team.js';
/**
 ...
 */
export function getValidBidActions(state, playerId) {
    const actions = [];
    // Always can pass
    ...
    if (state.highestBid) {
        ...
        actions.push({
            type: 'bid',
            bid: { playerId, points: 80, suit: undefined }, // Suit chosen by player
        });
    }
 ...
 */
export function isValidBidAction(action, state, playerSeat) {
    switch (action.type) {
        case 'pass':
            return true;
        case 'bid': {
...
export function isBiddingDead(state) {
